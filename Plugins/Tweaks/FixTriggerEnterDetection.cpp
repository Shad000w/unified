    ...
    +[](CNWSTrigger *thisPtr, uint32_t nEventId, OBJECT_ID nCallerObjectId, void* pScript, uint32_t nCalendarDay, uint32_t nTimeOfDay) -> void
    {
        if (nEventId == AIMasterEvent::SignalEvent)
        {
            ...
            if (pScriptEvent->m_nType == ScriptEvent::OnObjectEnter)
            {
				...
				auto *pClient = Globals::AppManager()->m_pServerExoApp->m_pcExoAppInternal->GetClientObjectByObjectId(pScriptEvent->GetObjectID(0));
				if (pEntered != nullptr && (!pClient || !pClient->GetIsDM()) && !VectorInTriggerBounds(pThis, pEntered->m_vPosition))
                {
                    delete pScriptEvent;
                    return;
                }
